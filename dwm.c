...
static void focusfullscreen(const Arg *arg);
...
static int isclientmaster(Client *client);
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(stext) - lrpad + statusrpad; /* right padding on status text */
		drw_text(drw, m->ww - tw, 0, tw, bh, 0, stext, 0);
		tw += statuslpad; /* left padding on status text */

		/* Draw status left padding rectangle */
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_rect(drw, m->ww - tw, 0, statuslpad, bh, 1, 1);
	}
...
isclientmaster(Client *client)
{
	...
	int i;
	Client *c;
	for (i = 0, c = selmon->clients; c && c != client; c = nexttiled(c->next), i++);

	return i < selmon->nmaster;
}
